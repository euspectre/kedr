...
enum calc_essence_type
{
    ...
    calc_essence_type_weak_variable,
...
//Essence which evaluated as weak variable with given index
struct calc_essence_weak_var
{
    struct calc_essence base;
    unsigned int index;
};
...
//Auxiliary structure, joined all data needed for evaluating expression
struct evaluate_data
{
    const kedr_calc_int_t* var_values;
    const struct kedr_calc_weak_var* weak_vars;
};

// Evaluate(possibly, recursively) essence, using given values of variables and weak_variables computation functions
static kedr_calc_int_t calc_essence_evaluate(const struct calc_essence* essence, const struct evaluate_data* evaluate_data);
...
struct kedr_calc
{
    //reference to the top-most essence
...
enum token_type
{
    ...
    token_type_weak_variable, // weak variable
...
struct parse_data
{
    ...
    union
    {
        ...
        int current_token_index;//for variable and weak variable token types
    };
};
...
parse_data_search_weak_variable(const struct parse_data* data,
    const char* weak_variable_name, int weak_variable_name_len, int* index);
...
{
    struct evaluate_data evaluate_data;
    evaluate_data.var_values = var_values;
    evaluate_data.weak_vars = calc->weak_vars;
    //
    return calc_essence_evaluate(calc->top_essence, &evaluate_data);
}
...
calc_essence_weak_var_create(unsigned int index)
{
    ...
    if(result == NULL) return NULL;
    result->base.type = calc_essence_type_weak_variable;
    result->index = index;
    return (struct calc_essence*)result;
}
...
calc_essence_evaluate(const struct calc_essence* essence, const struct evaluate_data* evaluate_data)
{
    ...
    case calc_essence_type_variable:
        return evaluate_data->var_values[((struct calc_essence_var*)essence)->index];
    case calc_essence_type_weak_variable:
        return evaluate_data->weak_vars[((struct calc_essence_weak_var*)essence)->index].compute();
// Helper macro for calculate value of operand for one-operand essence
#define OP1_1 (calc_essence_evaluate(((struct calc_essence_1op*)(essence))->op, evaluate_data))
    case calc_essence_type_unary_plus:
        return OP1_1;
    case calc_essence_type_unary_minus:
        return -OP1_1;
...
#define OP2_1 (calc_essence_evaluate(((struct calc_essence_2op*)(essence))->op1, evaluate_data))
#define OP2_2 (calc_essence_evaluate(((struct calc_essence_2op*)(essence))->op2, evaluate_data))
    case calc_essence_type_multiply:
        return OP2_1 * OP2_2;
    case calc_essence_type_divide:
        return OP2_1 / OP2_2;
...
#define OP3_1 (calc_essence_evaluate(((struct calc_essence_3op*)(essence))->op1, evaluate_data))
#define OP3_2 (calc_essence_evaluate(((struct calc_essence_3op*)(essence))->op2, evaluate_data))
#define OP3_3 (calc_essence_evaluate(((struct calc_essence_3op*)(essence))->op3, evaluate_data))
    case calc_essence_type_cond:
...
parse_data_next_token(struct parse_data* data)
{
    ...
    if(isalpha(ch) || (ch == '_'))
    {
        ...
        else if(!parse_data_search_weak_variable(data, name, name_len, &data->current_token_index))
        {
            data->current_token_type = token_type_weak_variable;
        }
        else
        {
            print_error("Name '%.*s' doesn't correspond to the name of the constant or variable.",
                name_len, name);
    ...
    else if(isdigit(ch))
    {
        ...
        //Determine type of number and look for the end of it
        if(number[0] == '0' && ((number[1] == 'x') || (number[1] == 'X')))
        {
            result = parse_number16(number + 2,
                &number_end, &data->current_token_value);
        }
        else
        {
            result = parse_number10(number,
                &number_end, &data->current_token_value);
        }
        if(result)
        {
        ...
        else
        {
            ...
            data->current_pos = number_end - data->expr;
        }
    }
...
// helpers for get additional attributes for token
static int is_substr_equal(const char* str, int len, const char* orig)
{
    ...
    const char* constant_name, int constant_name_len, kedr_calc_int_t* value)
{
    ...
    for(vector_index = 0; vector_index < data->const_vec_n; vector_index++)
    {
        ...
        for(constant_index = 0; constant_index < vec->n_elems; constant_index++)
        {
            ...
            if(is_substr_equal(constant_name, constant_name_len, const_def->name))
            {
    ...
    const char* variable_name, int variable_name_len, int* index)
{
    ...
    {
        if(is_substr_equal(variable_name, variable_name_len, data->var_names[var_index]))
        {
             *index = var_index;
             return 0;
        }
...
{
    int var_index;
    for(var_index = 0; var_index < data->weak_vars_n; var_index++)
    {
        if(is_substr_equal(variable_name, variable_name_len, data->weak_vars[var_index].name))
        {
             *index = var_index;
             return 0;
        }
    ...
    const char** number_end, kedr_calc_int_t* value)
{
    ...
    kedr_calc_int_t value_real = 0;
    if(!isdigit(*number))
    {
        print_error("Invalid first symbol of decimal number: '%c'",
            *number);
        return -1;
    }
    
    for(value_real = *number - '0', number_end_real = number + 1;
        isdigit(*number_end_real);
        number_end_real++)
    {
        // currently do not verify overflow
        value_real = value_real * 10 + (*number_end_real - '0');
    }
    *number_end = number_end_real;
    *value = value_real;
    return 0;
}
    ...
    const char** number_end, kedr_calc_int_t* value)
{
    ...
    if(!isxdigit(*number))
    {
        print_error("Invalid first symbol of hexadecimal number: '%c'",
            *number);
        return -1;
    }
#define HDIGIT_TO_VALUE(digit) (isdigit(digit) ? (digit) - '0' : \
    digit - (islower(digit) ? 'a' : 'A') + 10)

    for(value_real = HDIGIT_TO_VALUE(*number), number_end_real = number + 1;
        isxdigit(*number_end_real);
        number_end_real++)
    {
        // currently do not verify overflow
        value_real = value_real * 16 + HDIGIT_TO_VALUE(*number_end_real);
    }
#undef HDIGIT_TO_VALUE
