	...
	loff_t *f_pos)
{
    ...
    current_value = kedr_fsim_simulate(read_point, NULL);
    return count;
}
	...
	loff_t *f_pos)
{
    ...
    //current_value = kedr_fsim_simulate(write_point, &count);
    current_value = kedr_fsim_simulate(write_point, &my_count);
