...
#define print_error(format, ...) fprintf(stderr, "kedr_capture_trace: "format"\n", __VA_ARGS__)
...
static const char* short_options = "d:sf:p:";
static const char* short_options_h = "d:sf:p:h";
...
)
{
    ...
    *debugfs_mount_point = DEFAULT_DEBUGFS_MOUNT_POINT;
    ...
    opt = getopt_long(argc, argv, short_options_h, long_options, NULL);
    if(opt == 'h')
    {
