...
struct timer_list some_timer;

struct tasklet_struct some_tasklet;
...
static void cfake_setup_cdevice(struct cfake_dev *dev, int index)
{
	...
	if (err)
	{
		printk(KERN_NOTICE "[cp_target] Error %d while trying to add cfake%d",
			err, index);
	}
	else
	{
...
/* How many bytes to allocate while in atomic context.
 * cfake_ndevices is here to make the system call __kmalloc: if size is
 * a built-in constant, kmem_cache_alloc is used instead.
 */
#define CFAKE_ATOMIC_ALLOC_MEM (2 * sizeof(long) + cfake_ndevices)

static unsigned long
cfake_do_in_atomic(void)
{
	long* ptr;
	ptr = (long*)kmalloc(CFAKE_ATOMIC_ALLOC_MEM, GFP_ATOMIC);
	if (ptr != NULL) {
		*ptr = CFAKE_IOCTL_MAGIC;
		ptr[1] = *ptr;
		kfree(ptr);
	}
...
/* Do something with a spinlock - for testing purposes */
static void
cfake_spinlock_helper(spinlock_t* lock)
{
	static unsigned long flags;
	unsigned long result;
	
	spin_lock_irqsave(lock, flags);
	result = cfake_do_in_atomic();
	spin_unlock_irqrestore(lock, flags);
	
	/*printk(KERN_WARNING "[cp_target] cfake_spinlock_helper: "
		"result = %lu\n",
		result
	);*/
...
/* A handler for timer events and a function for tasklets at the same time
 */
static void
cfake_some_handler(unsigned long data)
{
	(void)data;
	cfake_spinlock_helper(&some_lock);
	return;
}
...
cfake_cleanup_module(void)
{
	...
	printk(KERN_ALERT "[cp_target] Cleaning up\n");
...
cfake_init_module(void)
{
	...
	printk(KERN_ALERT "[cp_target] Initializing\n");
	
	spin_lock_init(&some_lock);
	init_timer(&some_timer);
	tasklet_init(&some_tasklet, cfake_some_handler, 0);
	
	if (cfake_ndevices <= 0)
	{
		printk(KERN_WARNING "[cp_target] Invalid value of cfake_ndevices: %d\n", 
			cfake_ndevices);
		result = -EINVAL;
		return result;
	}
	...
	*/
	...
	if (result < 0) {
		printk(KERN_WARNING "[cp_target] can't get major number %d\n", cfake_major);
		return result;
	}
...
cfake_open(struct inode *inode, struct file *filp)
{
	...
	{
		printk(KERN_WARNING "[cp_target] No device found with MJ=%d and MN=%d\n", 
			mj, mn);
	...
	{
		printk(KERN_WARNING "[cp_target] open: internal error\n");
		return -ENODEV; /* No such device */
	}
	
	/* Testing spinlock operations */
	cfake_spinlock_helper(&some_lock);
	...
	if (dev->data == NULL)
	{
		...
		if (dev->data == NULL)
		{
			printk(KERN_WARNING "[cp_target] open: out of memory\n");
			return -ENOMEM;
		}
		memset(dev->data, 0, dev->buffer_size);
		
		/* Set a timer - just for testing  */
		del_timer(&some_timer); /* just in case */
		some_timer.data = 0;
		some_timer.function = cfake_some_handler;
		j = jiffies;
		some_timer.expires = j + 2 * HZ; /* 2 seconds */
		add_timer(&some_timer);
		/* ---- */
	}
...
cfake_release(struct inode *inode, struct file *filp)
{
	printk(KERN_WARNING "[cp_target] release() for MJ=%d and MN=%d\n", 
	...
	loff_t *f_pos)
{
	...
	printk(KERN_WARNING "[cp_target] read() for MJ=%d and MN=%d\n", 
	...
	loff_t *f_pos)
{
	...
	printk(KERN_WARNING "[cp_target] write() for MJ=%d and MN=%d\n", 
		imajor(filp->f_dentry->d_inode), iminor(filp->f_dentry->d_inode));
                 ...
                 unsigned int cmd, unsigned long arg)
{
	...
	printk(KERN_WARNING "[cp_target] ioctl() for MJ=%d and MN=%d\n", 
		imajor(inode), iminor(inode));
