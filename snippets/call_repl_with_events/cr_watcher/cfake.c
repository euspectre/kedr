...
#include "trace-events-cr_watcher.h" /* trace event facilities */
...
/*
 * [NB] copy_from_user() can be inline on some systems (kernel 2.6.34 or 
 * newer?), _copy_from_user() should probably be processed there instead.
 * 
 * [NB] __copy_from_user() is a whole another story, it may co-exist with
 * copy_from_user() and _copy_from_user().
 * */
...
repl___kmalloc(size_t size, gfp_t flags)
{
	...
	trace_called___kmalloc(size, flags, result);
...
repl_kmem_cache_alloc(struct kmem_cache* mc, gfp_t flags)
{
	...
	/* Here is our tracepoint for this function */
	trace_called_kmem_cache_alloc(mc, flags, result);
...
repl_kmem_cache_free(struct kmem_cache* mc, void* p)
{
	...
	/* Here is our tracepoint for this function */
	trace_called_kmem_cache_free(mc, p);
...
repl_copy_to_user(void __user * to, const void* from, unsigned long n)
{
	...
	/* Here is our tracepoint for this function */
	trace_called_copy_to_user(to, from, n, result);
